...
#### TESTS #### 
@pytest.mark.anyio
def test_get_all_users(mocker, user, user2, token):
...
@pytest.mark.anyio
def test_get_all_users_role_fail(mocker, token):
...
@pytest.mark.anyio
def test_get_all_users_fail(mocker, token):
    mock_service_get_all_users = mocker.patch('src.api.components.users.controller.user_service.get_all_users')
    mock_service_roles_required = mocker.patch('src.api.components.users.controller.roles_required')
    mock_service_roles_required.return_value = None    
...
@pytest.mark.anyio
def test_get_user_by_id(mocker, token, user):
...
@pytest.mark.anyio
def test_get_user_by_id_role_fail(mocker, token):
    mock_service_roles_required = mocker.patch('src.middleware.role_auth.roles_required')
    mock_service_get_user_by_id = mocker.patch('src.api.components.users.controller.user_service.get_user_by_id')
    with pytest.raises(HTTPException) as exception:
        controller.get_all_users(token) 
...
@pytest.mark.anyio
def test_get_user_by_id_fail(mocker, token):
    mock_service_get_user_by_id = mocker.patch('src.api.components.users.controller.user_service.get_user_by_id')
    mock_service_roles_required = mocker.patch('src.api.components.users.controller.roles_required')
...
@pytest.mark.anyio
def test_get_user_by_email(mocker, token, user):
    ...
    result = controller.get_user_by_email(token)   
    assert mock_get_user_by_email.called_once
    assert result == user

@pytest.mark.anyio
def test_get_user_by_email_role_fail(mocker, token):
    mock_service_roles_required = mocker.patch('src.middleware.role_auth.roles_required')
...
@pytest.mark.anyio
def test_get_user_by_email_fail(mocker, token):
    mock_service_get_user_by_email =  mocker.patch('src.api.components.users.controller.user_service.get_user_by_email')
    mock_service_roles_required = mocker.patch('src.api.components.users.controller.roles_required')
    mock_service_roles_required.return_value = None
    mock_service_get_user_by_email.return_value = None
    with pytest.raises(HTTPException) as exception:
        controller.get_user_by_email(token)   
...
@pytest.mark.anyio
def test_create_register_submition_email_exists(mocker, user_register):
...
@pytest.mark.anyio
def test_create_register_submition_fail(mocker, user_register):
    # Mocks
...
@pytest.mark.anyio
def test_confirm_user(mocker):
    mocker.patch('src.api.components.users.controller.user_service.confirm_user', return_value = True)
    mock_service_roles_required = mocker.patch('src.api.components.users.controller.roles_required')
    mock_service_roles_required.return_value = None 
    confirmation_code = 1234
    result = controller.confirm_user(confirmation_code)
    assert result == True

@pytest.mark.anyio
def test_confirm_user_role_fail(mocker, token):
    mock_service_roles_required = mocker.patch('src.middleware.role_auth.roles_required')
    ...
    assert exception.value.detail == 'Access denied'
    assert mock_service_roles_required.called_once
    assert not mock_service_confirm_user.called

@pytest.mark.anyio
def test_confirm_user_fail(mocker):
    mocker.patch('src.api.components.users.controller.user_service.confirm_user', side_effect=Exception)
    mock_service_roles_required = mocker.patch('src.api.components.users.controller.roles_required')
    mock_service_roles_required.return_value = None
    with pytest.raises(HTTPException) as exception_info:
        controller.confirm_user(1234)
    assert isinstance(exception_info.value, HTTPException)
...
def test_forgot_password(mocker, user_email):
    ...
    assert mock_service_forgot_password.called_once
